...
openssl req -config ${CSR_CONFIG} -newkey rsa:4096 -nodes -keyout ${CERT_CN}.key -out ${CERT_CN}.csr
openssl x509 -req -days 365 -in ${CERT_CN}.csr -CA rootCA.crt -CAkey rootCA.key -CAcreateserial -out ${CERT_CN}.crt
openssl pkcs12 -export -out ${CERT_CN}.p12 -inkey ${CERT_CN}.key -in ${CERT_CN}.crt  -name axonserver -passout pass:axonserver
