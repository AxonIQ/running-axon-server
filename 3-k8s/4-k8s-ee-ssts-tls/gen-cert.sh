...
Usage () {
    ...
    echo "  --ca <name>       Prefix for the internal CA files, default 'ca'."
    echo "  --output <name>   Base name of the generated files, default 'ssl'. Shorthand option '-o'."
    exit 1
}

options=$(getopt -l 'country:,state:,city:,org:,dn:,output:,ca-dir:' -- 'c:o:' "$@")
[ $? -eq 0 ] || {
    Usage
}
eval set -- "$options"
